...
static void sf_tls_recv_cb(sock_session_t* ss) {
	...
	do {
		...
		//if (SSL_do_handshake(ssl)) {
			...
			if (rd <= 0) {
				serr = sf_tls_read_err(ss, rd, &rt, errstr);
				//预期内的错误
				if (serr == SERROR_OK)
					return;
...
static int32_t sf_tls_send_err(sock_session_t* ss, int32_t sd, _OUT int32_t* errcode, _OUT int32_t* openssl_err, _OUT char* errstr) {
	int rt, err, eno, serr = 0;
#if (ENABLE_SSL)
	SSL* ssl = (SSL*)(ss->tls_info.ssl);
