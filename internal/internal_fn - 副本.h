...
static void sf_recv_cb(sock_session_t* ss) {
	...
	do {
		...
		int rd = recv(ss->fd, (char*)RWBUF_START_PTR(&(ss->rbuf)) + RWBUF_GET_LEN(&(ss->rbuf)), buflen, 0);
		if (rd == -1) {
#ifndef _WIN32
			eno = errno;
...
static int32_t sf_tls_read_err(sock_session_t* ss, int32_t rd, _OUT int32_t* errcode, _OUT int32_t* openssl_err, _OUT char* errstr) {
...
static void sf_tls_recv_cb(sock_session_t* ss) {
...
#if (ENABLE_SSL)
	int32_t rt = 0, err, eno, rd, opensslerr = 0, serr = 0;
	SSL* ssl = (SSL*)(ss->tls_info.ssl);
	...
	case SERROR_SYSAPI_ERR:
		printf("[%s] [%s:%d] [%s], Ready to disconnect, ip: [%s] port: [%d], errno: [%d], msg: [%s]\n", sf_timefmt(), __FILENAME__, __LINE__, __FUNCTION__, ss->ip, ss->port, eno, "System api error");
		break;
	...
	default:
		...
		printf("[%s] [%s:%d] [%s], Ready to disconnect, ip: [%s] port: [%d], serr: [%d], errno: [%d], rstcode: [%d], msg: [%s]\n", sf_timefmt(), __FILENAME__, __LINE__, __FUNCTION__, ss->ip, ss->port, serr, eno, rt, errstr);
	}
...
static int32_t sf_tls_send_err(sock_session_t* ss, int32_t sd, _OUT int32_t* out_tls_err, _OUT int32_t* openssl_err, _OUT char* errstr) {
	int rt, err, eno, serr = 0;
#if (ENABLE_SSL)
	SSL* ssl = (SSL*)(ss->tls_info.ssl);
	//rt = sf_tls_err(ssl, sd);
	*out_tls_err = rt = SSL_get_error(ssl, sd);
	err = ERR_get_error();
	ERR_clear_error();
	...
	if (rt = SSL_ERROR_WANT_READ && ss->flag.tls_handshake == 0) {
...
static void sf_tls_send_cb(sock_session_t* ss) {
...
#if (ENABLE_SSL)
	int32_t rt, snd_len, eno, sd, opensslerr = 0, serr = 0, err = 0;
	SSL* ssl = (SSL*)ss->tls_info.ssl;
	static char errstr[512];
	errstr[0] = 0;
	...
	do {
		if (ss->flag.tls_handshake) {
			...
			if (sd <= 0) {
				//写判断错误
				serr = sf_tls_send_err(ss, sd, &rt, &opensslerr, errstr);
				//预期内的错误
				if (serr == SERROR_OK)
					return;
...
static int32_t sf_tls_enable_from_ctx(sock_session_t* ss, void* tls_ctx, _OUT char* errstr) {
#if (ENABLE_SSL)
	int32_t fd = ss->fd;
	int32_t err = 0, rt;
