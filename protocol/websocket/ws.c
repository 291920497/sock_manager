...
extern void sf_set_ready(sock_session_t* ss, uint8_t ready);
extern uint8_t sf_get_ready(sock_session_t* ss);
...
static int sf_ws_handshake(sock_session_t* ss, const char* url, const char* host, const char* origin, const char* sec_key, const char* sec_version) {
	...
	base64_encode_r(sha1, 20, b64, sizeof(b64));
...
int32_t ws_decode_cb(sock_session_t* ss, char* data, uint32_t data_len, rcv_decode_mod_t* mod, uint32_t* offset) {
	...
	//如果已经完成握手
	if (sf_get_ready(ss)) {
		return sf_ws_parse_frame(ss, data, data_len, mod, offset);
	}
	else {
		if (data_len > 4) {
