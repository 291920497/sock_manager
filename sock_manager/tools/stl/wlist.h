...
#define cds_list_for_each_entry(pos, head, member) \
	for (pos = cds_list_entry((head)->next, std::remove_reference<decltype(*(pos))>::type, member); \
		&(pos)->member != (head); \
		pos = cds_list_entry((pos)->member.next, std::remove_reference<decltype(*(pos))>::type, member))

#define cds_list_for_each_entry_reverse(pos, head, member) \
	for (pos = cds_list_entry((head)->prev, std::remove_reference<decltype(*(pos))>::type, member); \
		&(pos)->member != (head); \
		pos = cds_list_entry((pos)->member.prev, std::remove_reference<decltype(*(pos))>::type, member))

#define cds_list_for_each_entry_safe(pos, p, head, member) \
	for (pos = cds_list_entry((head)->next, std::remove_reference<decltype(*(pos))>::type, member), \
			p = cds_list_entry((pos)->member.next, std::remove_reference<decltype(*(pos))>::type, member); \
		&(pos)->member != (head); \
		pos = (p), p = cds_list_entry((pos)->member.next, std::remove_reference<decltype(*(pos))>::type, member))
...
#define cds_list_for_each_entry_safe_from(pos, p, head, member) \
        for (p = cds_list_entry((pos)->member.next, std::remove_reference<decltype(*(pos))>::type, member); \
                &(pos)->member != (head); \
                pos = (p), p = cds_list_entry((pos)->member.next, std::remove_reference<decltype(*(pos))>::type, member))
