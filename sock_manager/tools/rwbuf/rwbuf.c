...
int32_t rwbuf_append_complete(rwbuf_t* rwb, const void* data, uint32_t data_len) {
	if (rwbuf_enough(rwb, data_len)) {
		return rwbuf_append(rwb, data, data_len);
	}

	int32_t rt = rwbuf_relc(rwb, rwbuf_capcity(rwb) + data_len - rwbuf_unused_len(rwb));
	if (rt != SERROR_OK)
		return SERROR_SYSAPI_ERR;

	rwbuf_append(rwb, data, data_len);
	return data_len;
}
